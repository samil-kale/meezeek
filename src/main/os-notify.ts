...
function buildWindowsCommand(storageDir: string, id: string, title: string, body: string): string {
  ...
  // "Windows PowerShell" rather than to meezeek — an app identity of our own would have to
...
function buildMacCommand(storageDir: string, id: string, title: string, body: string): string {
  ...
  writePosixScript(
    scriptFile,
    `#!/bin/sh
MEEZEEK_TITLE=${shellSingleQuote(title)} MEEZEEK_BODY=${shellSingleQuote(body)} osascript -e 'display notification (system attribute "MEEZEEK_BODY") with title (system attribute "MEEZEEK_TITLE")' >/dev/null 2>&1
exit 0
`
  );
