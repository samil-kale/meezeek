...
/**
 * Every operation here goes through the one `opencode serve` instance meezeek runs per
...
export const opencodeSessionProvider: SessionProvider = {
  async list(executable: string, cwd: string): Promise<AgentSessionInfo[]> {
    try {
      ...
      console.error("[meezeek] opencode session listing failed:", error);
      return [];
    }
  },
  ...
  watch(executable: string, cwd: string, onChange: () => void): () => void {
    ...
    void ensureServer(executable, cwd)
      ...
      .catch((error) => console.error("[meezeek] opencode event stream unavailable:", error));
    return () => {
      stopped = true;
      unsubscribe?.();
    };
